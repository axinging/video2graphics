...
function renderWithWebGL2(gl, program, videoFrame, resources) {
  const { positionBuffer, videoTexture,
    positionLocation, texCoordLocation, imageLocation,
    webglCanvas } = resources;
...
export function createWebGL2BlurRenderer(segmenterFunction, params) {
  ...
  const fragmentShaderSourceBlur = `#version 300 es
      precision highp float;
      in vec2 v_texCoord;
      out vec4 fragColor;
      uniform sampler2D u_image;
      ...
      void main() {
        ...
        fragColor = blurredColor;//mix(blurredColor, originalColor, mask);
      }
    `;
  ...
  return {
    render: async (videoFrame) => {
      ...
      // gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, videoFrame);
      // Segment
      const maskImageData = null;

      // WebGL2 rendering implementation with actual blur effect
      renderWithWebGL2(gl, program, videoFrame, {
        positionBuffer, videoTexture,
        positionLocation, texCoordLocation, imageLocation,
        webglCanvas
      });
