...
export async function createWebGL2BlurRenderer(segmenterFunction, params) {
  ...
  const fragmentShaderSource = `#version 300 es
      ...
      ${enableDownscale ? "uniform sampler2D u_mask;" : ""}
      ...
      void main() {
        ...
        ${enableDownscale ? "float mask = texture(u_mask, v_texCoord).r; fragColor = mix(blurredColor, originalColor, mask);" : "fragColor = blurredColor; return;"}
  ...
  return {
    render: async (videoFrame) => {
      ...
      if (enableDownscale) {
        ...
        // WebGL2 rendering implementation with actual blur effect
        renderWithWebGL2(gl, program, videoFrame, null, {
          positionBuffer, videoTexture, maskTexture: null,
          positionLocation, texCoordLocation, imageLocation,
          maskLocation: null, resolutionLocation, blurAmountLocation,
          webglCanvas
        });
