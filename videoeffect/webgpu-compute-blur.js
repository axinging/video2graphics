...
async function renderWithWebGPU(params, videoFrame, resourceCache) {
  ...
  const bindGroup = enableDownscale ? device.createBindGroup({
...
export async function createWebGPUBlurRenderer(segmenter, params) {
  ...
  const tileSize = 8 + 2 * kernelRadius; // workgroup_size + 2*radius

  const computeShaderCode2 = `
  @group(0) @binding(0) var inputTexture: ${zeroCopy ? "texture_external" : "texture_2d<f32>"};
  @group(0) @binding(1) var outputTexture: texture_storage_2d<${getTextureFormat(directOutput)}, write>;
  @group(0) @binding(2) var textureSampler: sampler;
  ...
  var<workgroup> tile: array<array<vec4<f32>, ${tileSize}>, ${tileSize}>;
  ...
  fn main(
    ...
    let inputDims = textureDimensions(inputTexture);
    let _unused = uniforms;

    for (var dy = 0u; dy < ${tileSize}; dy += 8u) {
      for (var dx = 0u; dx < ${tileSize}; dx += 8u) {
        let tileY = local_id.y + dy;
        let tileX = local_id.x + dx;
        if (tileY < ${tileSize} && tileX < ${tileSize}) {
          // 
          let globalY = i32(global_id.y) + i32(tileY) - ${kernelRadius};
          let globalX = i32(global_id.x) + i32(tileX) - ${kernelRadius};
          // 
          //let sampleY = clamp(globalY, 0, inputDims.y - 1);
          //let sampleX = clamp(globalX, 0, inputDims.x - 1);
          let sampleY = clamp(globalY, 0, i32(inputDims.y) - 1);
          let sampleX = clamp(globalX, 0, i32(inputDims.x) - 1);
          let uv = (vec2<f32>(f32(sampleX), f32(sampleY)) + vec2<f32>(0.5, 0.5)) / vec2<f32>(inputDims);
          tile[tileY][tileX] = textureSampleBaseClampToEdge(inputTexture, textureSampler, uv);
        }
      }
    }
    workgroupBarrier();
    ...
    for (var dx = -${kernelRadius}; dx <= ${kernelRadius}; dx++) {
      for (var dy = -${kernelRadius}; dy <= ${kernelRadius}; dy++) {
        //let tileX = local_id.x + ${kernelRadius} + dx;
        //let tileY = local_id.y + ${kernelRadius} + dy;
        let tileX = i32(local_id.x) + ${kernelRadius} + dx;
        let tileY = i32(local_id.y) + ${kernelRadius} + dy;
        
        let tileXc = clamp(tileX, 0, ${tileSize} - 1);
        let tileYc = clamp(tileY, 0, ${tileSize} - 1);
        let weight = 1.0 / (1.0 + length(vec2<f32>(f32(dx), f32(dy))));
        blurredColor += tile[tileYc][tileXc] * weight;
        totalWeight += weight;
      }
    }
    if (totalWeight > 0.0) {
